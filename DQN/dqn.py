...
EPSILON = 0.9
GAMMA = 0.9
TARGET_REPLACE_ITER = 100  # target Q function的参数更新频率
...
class DQN(object):
    def __init__(self):
        ...
        self.memory_counter = 0  # 经验buffer中位值的计数器
        self.memory = np.zeros((MEMORY_CAPACITY, N_STATES * 2 + 2))  # 初始化经验buffer
        # 经验buffer初始化为全0，存储两个state的数值加上一个a(action)和一个r(reward)的数值
    ...
    def choose_action(self, x):
        ...
        # 经验buffer，存储之前的经验，学习之前的经验buffer里的东西
...
for i_episode in range(400):
    ...
    while True:
        ...
        s_, r, done,_, info = env.step(a)  # 根据环境的行为，给出一个反馈
